 ...
 public:
  ...
  T GetElement(int i, int j) const override {
    if (i < 0 || i >= this->GetRowCount() || j < 0 || j >= this->GetColumnCount()) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "get : out of range");
    }
    return data_[i][j];
  }
  ...
  void SetElement(int i, int j, T val) override {
    if (i < 0 || i >= this->GetRowCount() || j < 0 || j >= this->GetColumnCount()) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "set : out of range");
    }
    data_[i][j] = val;
  }
  ...
  void FillFrom(const std::vector<T> &source) override {
    ...
    if (static_cast<int>(source.size()) != row * col) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "fill : out of range");
    }
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    RowMatrix<T> *q = new RowMatrix<T>(row, col);
    for (int i = 0; i < row; i++) {
      for (int j = 0; j < col; j++) {
        T temp = matrixA->GetElement(i, j) + matrixB->GetElement(i, j);
        q->SetElement(i, j, temp);
      }
    }

    return std::unique_ptr<RowMatrix<T>>(q);
  }
  /**
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    RowMatrix<T> *q = new RowMatrix<T>(matrix_a_row, matrix_b_col);
